...
const STEP_KEYS = ['access', 'automation', 'whatsapp'] as const;

function StepAccessVisual() {
  const t = useTranslations('home.howItWorks.steps.access');
  return (
    <div className="relative flex h-full min-h-[200px] flex-col justify-end overflow-hidden rounded-2xl bg-gradient-to-br from-primary-50 to-white p-4 dark:from-primary-950/50 dark:to-neutral-900 sm:min-h-[220px]">
      ...
      <div className="relative mt-3 space-y-2">
        <div className="flex items-center justify-between rounded-xl bg-white px-3 py-2.5 ring-1 ring-neutral-200 dark:bg-neutral-800 dark:ring-neutral-700">
          <span className="text-xs text-neutral-800 dark:text-neutral-100">{t('notification')}</span>
        ...
        <div className="flex gap-2">
          {(['night', 'weekend', 'holiday'] as const).map((key) => (
            <span
              key={key}
...
function StepAutomationVisual() {
  const t = useTranslations('home.howItWorks.steps.automation');
  const rows = ['availability', 'conflict', 'sync'] as const;
  return (
    <div className="relative flex h-full min-h-[200px] flex-col overflow-hidden rounded-2xl bg-gradient-to-br from-primary-50 to-white p-4 dark:from-primary-950/50 dark:to-neutral-900 sm:min-h-[220px]">
      ...
      <p className="mt-1 text-sm font-semibold text-neutral-900 dark:text-white">{t('heading')}</p>
...
function StepWhatsAppVisual() {
  const t = useTranslations('home.howItWorks.steps.whatsapp');
  return (
...
const STEP_VISUALS = {
  access: StepAccessVisual,
  automation: StepAutomationVisual,
  whatsapp: StepWhatsAppVisual,
} as const;
...
export function HomeHowItWorks() {
  ...
  return (
    <AnimateIn as="section" animation="slide-up" aria-labelledby="home-solution-title">
      <HomeSectionHeader
        eyebrow={t('eyebrow')}
        title={t('title')}
      ...
      <ol className="mx-auto mt-10 flex max-w-4xl flex-col gap-6 sm:mt-12 sm:gap-8 lg:gap-10">
        {STEP_KEYS.map((key, i) => {
          const Visual = STEP_VISUALS[key];
          return (
            <AnimateIn key={key} animation="slide-up" delay={i * 90} as="li">
